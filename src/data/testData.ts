interface IRoleUserGroup {
  functionGroupID: number;
  functionIndex: number;
  functionID: number;
  functionName: string;
  isPermission: boolean;
}
interface IRoleDetail {
  functionGroupName: string;
  userGroupFunctions: IRoleUserGroup[];
}
interface IRoleResponse {
  userGroupID: string;
  userGroupName: string;
  status: boolean;
  functions: IRoleDetail[];
}

export const testData: IRoleResponse = {
  userGroupID: "ADMIN",
  userGroupName: "Admin",
  status: false,
  functions: [
    {
      functionGroupName: "Quản lý nhóm người dùng",
      userGroupFunctions: [
        {
          functionGroupID: 5,
          functionIndex: 22,
          functionID: 11,
          functionName: "Tất cả",
          isPermission: true,
        },
        {
          functionGroupID: 5,
          functionIndex: 23,
          functionID: 12,
          functionName: "Sửa",
          isPermission: true,
        },
        {
          functionGroupID: 5,
          functionIndex: 24,
          functionID: 13,
          functionName: "Xem",
          isPermission: true,
        },
        {
          functionGroupID: 5,
          functionIndex: 33,
          functionID: 14,
          functionName: "Đồng bộ",
          isPermission: true,
        },
      ],
    },
    {
      functionGroupName: "Quản lý người dùng",
      userGroupFunctions: [
        {
          functionGroupID: 6,
          functionIndex: 25,
          functionID: 11,
          functionName: "Tất cả",
          isPermission: true,
        },
        {
          functionGroupID: 6,
          functionIndex: 26,
          functionID: 12,
          functionName: "Sửa",
          isPermission: true,
        },
        {
          functionGroupID: 6,
          functionIndex: 27,
          functionID: 13,
          functionName: "Xem",
          isPermission: true,
        },
        {
          functionGroupID: 6,
          functionIndex: 28,
          functionID: 14,
          functionName: "Đồng bộ",
          isPermission: true,
        },
      ],
    },
    {
      functionGroupName: "Ngân hàng câu hỏi",
      userGroupFunctions: [
        {
          functionGroupID: 7,
          functionIndex: 29,
          functionID: 11,
          functionName: "Tất cả",
          isPermission: true,
        },
        {
          functionGroupID: 7,
          functionIndex: 42,
          functionID: 21,
          functionName: "Thêm",
          isPermission: true,
        },
        {
          functionGroupID: 7,
          functionIndex: 30,
          functionID: 12,
          functionName: "Sửa",
          isPermission: true,
        },
        {
          functionGroupID: 7,
          functionIndex: 31,
          functionID: 13,
          functionName: "Xem",
          isPermission: true,
        },
        {
          functionGroupID: 7,
          functionIndex: 45,
          functionID: 19,
          functionName: "Xóa",
          isPermission: true,
        },
        {
          functionGroupID: 7,
          functionIndex: 43,
          functionID: 22,
          functionName: "Upload",
          isPermission: true,
        },
        {
          functionGroupID: 7,
          functionIndex: 44,
          functionID: 23,
          functionName: "Sao chép",
          isPermission: true,
        },
      ],
    },
    {
      functionGroupName: "Bài khảo sát",
      userGroupFunctions: [
        {
          functionGroupID: 8,
          functionIndex: 34,
          functionID: 11,
          functionName: "Tất cả",
          isPermission: true,
        },
        {
          functionGroupID: 8,
          functionIndex: 35,
          functionID: 12,
          functionName: "Sửa",
          isPermission: true,
        },
        {
          functionGroupID: 8,
          functionIndex: 36,
          functionID: 13,
          functionName: "Xem",
          isPermission: true,
        },
        {
          functionGroupID: 8,
          functionIndex: 32,
          functionID: 15,
          functionName: "Xuất excel",
          isPermission: true,
        },
        {
          functionGroupID: 8,
          functionIndex: 37,
          functionID: 16,
          functionName: "Tạo bài khảo sát",
          isPermission: true,
        },
        {
          functionGroupID: 8,
          functionIndex: 38,
          functionID: 17,
          functionName: "Sao chép bài khảo sát",
          isPermission: true,
        },
        {
          functionGroupID: 8,
          functionIndex: 39,
          functionID: 18,
          functionName: "Sao chép link",
          isPermission: true,
        },
        {
          functionGroupID: 8,
          functionIndex: 40,
          functionID: 19,
          functionName: "Xóa",
          isPermission: true,
        },
        {
          functionGroupID: 8,
          functionIndex: 41,
          functionID: 20,
          functionName: "Khóa bài khảo sát",
          isPermission: true,
        },
      ],
    },
  ],
};

export interface IRolDataFormatted {
  string: number[];
}

export const formattedData = testData.functions.reduce((acc, cur) => {
  const roleInfo: number[] = [];
  cur.userGroupFunctions.forEach((el) => {
    if (el.isPermission) {
      roleInfo.push(el.functionID);
    }
  });
  return {
    ...acc,
    [cur.functionGroupName]: roleInfo,
  };
}, {}) as IRolDataFormatted;
